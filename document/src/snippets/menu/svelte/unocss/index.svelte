...
<div>
  <button
    {...api.getTriggerProps()}
    class="ml-4 items-center justify-center rounded-md
    text-sm font-medium ring-offset-background transition-colors
    disabled:pointer-events-none disabled:opacity-50 bg-primary!
    text-primary-foreground! hover:bg-primary/90! h-10 px-4 py-2 outline-none!"
  >
    Open
  </button>
  ...
  {#if api.open}
    <div use:portal>
      ...
      >
        ...
        <div class="-mx-1 my-1 h-px bg-muted" {...api.getSeparatorProps()} ></div>
        
        {#each items as itemList, index}
          <ul {...api.getContentProps()} class="group p-1 pb-0 last:pb-1 outline-none!">
