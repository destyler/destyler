...
import type { TrapEventMap } from './traps'
import { trapOff, trapOn } from './traps'
...
function createDelegate(): Delegate {
  ...
  function spy(event: Event, propName: keyof Event, fn: (...args: any[]) => any): Event {
    const source = event[propName];
    (event as any)[propName] = function (...args: any[]) {
      fn.apply(event, args)
      return (source as any).apply(event, args)
    } as any
    return event
  }
