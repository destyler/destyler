...
export function clone<T>(x: T): T {
  ...
  if (str === '[object Object]') {
    ...
    tmp = Array.from({ length: (x as any).length })
  }
  else if (str === '[object Set]') {
    tmp = new Set()
    ;(x as Set<any>).forEach((val: any) => {
      tmp.add(clone(val))
    })
  }
  else if (str === '[object Map]') {
    tmp = new Map()
    ;(x as Map<any, any>).forEach((val: any, key: any) => {
  ...
  if (tmp) {
    ...
    for (i = 0, list = Object.getOwnPropertyNames(x as object); i < list.length; i++) {
      if (Object.hasOwnProperty.call(tmp, (k = list[i])) && tmp[k] === (x as any)[k])
        continue
