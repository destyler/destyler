...
interface AsRef { $$valtioRef: true }
...
const proxyStateMap = globalRef('__destyler__proxyStateMap', () => new WeakMap<ProxyObject, ProxyState>())

function buildProxyFunction(objectIs = Object.is, newProxy = <T extends object>(target: T, handler: ProxyHandler<T>): T => new Proxy(target, handler), snapCache = new WeakMap<object, [version: number, snap: unknown]>(), createSnapshot: CreateSnapshot = <T extends object>(target: T, version: number): T => {
...
}, proxyCache = new WeakMap<object, ProxyObject>(), versionHolder = [1, 1] as [number, number], proxyFunction = <T extends object>(initialObject: T): T => {
