 ...
 */
import { getDocument, getEventTarget, getWindow, isMac } from '@destyler/dom'
...
export const listenerMap = new Map<Window, GlobalListenerData>()
...
function tearDownWindowFocusTracking(root?: RootNode, loadListener?: () => void) {
  const win = getWindow(root)
  const doc = getDocument(root)
