...
function getScrollPadding(element: HTMLElement): Record<ScrollAxis, { before: number, after: number }> {
  ...
  const xBefore = convert(xBeforeRaw, rect.width)
  const yBefore = convert(yBeforeRaw, rect.height)
  const xAfter = convert(xAfterRaw, rect.width)
  const yAfter = convert(yAfterRaw, rect.height)
