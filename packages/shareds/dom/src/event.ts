import type { MaybeFn } from '@destyler/types'
...
export function isSelfTarget(event: Partial<Pick<UIEvent, 'currentTarget' | 'target' | 'composedPath'>>) {
...
export function isContextMenuEvent(e: Pick<MouseEvent, 'button' | 'ctrlKey' | 'metaKey'>) {
  return e.button === 2 || (isMac() && e.ctrlKey && e.button === 0)
}

export function isModifierKey(e: Pick<KeyboardEvent, 'ctrlKey' | 'metaKey' | 'altKey'>) {
...
export function addDomEvent<K extends keyof DOMEventMap>(target: MaybeFn<EventTarget | null>, eventName: K, handler: (event: DOMEventMap[K]) => void, options?: boolean | AddEventListenerOptions) {
