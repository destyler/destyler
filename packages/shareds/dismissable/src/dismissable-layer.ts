import type { FocusOutsideEvent, InteractOutsideHandlers, PointerDownOutsideEvent } from '@destyler/interact-outside'
import type { MaybeFunction } from '@destyler/utils'
import type { Layer } from './layer-stack'
import { contains, getEventTarget, isHTMLElement, raf } from '@destyler/dom'
import { trackInteractOutside } from '@destyler/interact-outside'
import { isFunction } from '@destyler/utils'
...
function trackDismissableElementImpl(node: MaybeElement, options: DismissableElementOptions) {
  if (!node) {
    throw new Error('[@destyler/dismissable] node is `null` or `undefined`')
  }
...
export function trackDismissableBranch(nodeOrFn: NodeOrFn, options: { defer?: boolean } = {}) {
  ...
  cleanups.push(
    func(() => {
      const node = isFunction(nodeOrFn) ? nodeOrFn() : nodeOrFn
      if (!node) {
        throw new Error('[@destyler/dismissable] branch node is `null` or `undefined`')
      }
