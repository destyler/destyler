...
import { hasProp, isEqual, isObject } from '@destyler/utils'
...
export class ListCollection<T extends CollectionItem = CollectionItem> {
  ...
  private getByText(text: string, current: string | null): T | undefined {
    ...
    if (isSingleKey) {
      // For single character search, exclude current item to find next match
...
function match(label: string | null, query: string) {
  return !!label?.toLowerCase().startsWith(query.toLowerCase())
}

function wrap<T>(v: T[] | readonly T[], idx: number) {
