import type { IndexPath, TreeVisitOptions } from './tree-visit'
import type {
  FilePathTreeNode,
  FlatTreeNode,
...
import { compact, hasProp, isEqual, isObject } from '@destyler/utils'
import {
  access,
  compareIndexPaths,
  find,
...
const fallback: TreeCollectionMethods<TreeNode> = {
  nodeToValue(node) {
    if (typeof node === 'string')
      return node
    if (isObject(node) && hasProp(node, 'value'))
      return node.value
    return ''
  },
  nodeToString(node) {
    if (typeof node === 'string')
      return node
    if (isObject(node) && hasProp(node, 'label'))
      return node.label
...
export class TreeCollection<T = TreeNode> {
  ...
  getParentNodes = (values: string): T[] => {
    const result: T[] = []
    const indexPath = this.getIndexPath(values)
    if (!indexPath)
      return result
    ...
    while (pathCopy.length > 0) {
      pathCopy.pop()
      const parentNode = this.at(pathCopy)
  ...
  visit = (opts: Omit<TreeVisitOptions<T>, 'getChildren'> & TreeSkipOptions<T>) => {
    ...
    visit(this.rootNode, {
      ...
      onEnter: (node, indexPath) => {
        if (this.isRootNode(node))
          return
        if (skip?.({ value: this.getNodeValue(node), node, indexPath }))
          return 'skip'
