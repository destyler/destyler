...
function correctTargets(parent: HTMLElement, targets: Element[]): Element[] {
  return targets
    .map((target) => {
      ...
      console.error('[@destyler/aria-hidden] target', target, 'in not contained inside', parent, '. Doing nothing')
      return null
    })
...
export function walkTreeOutside(originalTarget: Element | Element[], props: WalkTreeOutsideOptions): VoidFunction {
    ...
    Array.prototype.forEach.call(parent.children, (node: Element) => {
      if (elementsToKeep.has(node)) {
        ...
        try {
          ...
          console.error('[@destyler/aria-hidden] cannot operate on ', node, e)
        }
      }
    })
  }
