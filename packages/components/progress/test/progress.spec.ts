...
const disabledComponent = testid('disabled-progress')
const readonlyComponent = testid('readonly-progress')
const invalidComponent = testid('invalid-progress')
const requiredComponent = testid('required-progress')
...
export async function RendersCorrectly() {
  ...
  await expect.element(componentEl).toHaveAttribute('data-scope', 'progress')
}
