...
const set = {
  value: (ctx: MachineContext, value: string | string[] | null | undefined, force = false) => {
    ...
    if (value == null) {
      ...
      next = ctx.multiple ? addOrRemove(ctx.value, value) : [value]
    }
...

    {
      ...
      activities: {
        ...
        scrollToHighlightedItem(ctx, _evt, { getState }) {
          ...
          const exec = (immediate: boolean) => {
            ...
            if (pointer || !ctx.highlightedValue)
              return
      ...
      actions: {
        ...
        autofillInputValue(ctx, evt) {
          const inputEl = dom.getInputEl(ctx)
          if (!ctx.autoComplete || !inputEl || !evt.keypress)
            return
