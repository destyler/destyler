...
To set the initial value **without** taking ownership of state, pass `defaultValue`. Prefer this over seeding with `value` (which is presence-controlled after Phase 2).

:::tip
The `defaultValue` / `value` property must be an array of strings. If selecting a single value, pass an array with a single string.
:::
...
Combobox supports controllable **value**, **inputValue**, and **open**. Use `defaultValue` / `defaultInputValue` / `defaultOpen` for uncontrolled seeds. Pass `value` / `inputValue` / `open` and sync with `onValueChange` / `onInputValueChange` / `onOpenChange` when your app owns state. See [Controllable State](/guide/controllable-state).

```ts
const value = ["vue"] // your reactive state
combobox.machine({
  id: "1",
  value,
