...
To make a hover card open by default **without** taking ownership of state, use `defaultOpen`. Prefer this over seeding with `open` (which is presence-controlled after Phase 2).
...
Hover card open state follows the same pattern as popover. Use `defaultOpen` for an uncontrolled seed; pass `open` + `onOpenChange` (and sync via `context`) when you need controlled open. See [Controllable State](/guide/controllable-state).
...
const open = false // your reactive state
hoverCard.machine({
  id: "1",
  open,
