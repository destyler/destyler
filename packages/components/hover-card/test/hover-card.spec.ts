...
const disabledComponent = testid('disabled-hover-card')
const nestedComponent = testid('nested-hover-card')
...
export async function RendersCorrectly() {
  ...
  await expect.element(componentEl).toHaveAttribute('data-scope', 'hover-card')
}
