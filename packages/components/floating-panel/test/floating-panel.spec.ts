...
const disabledComponent = testid('disabled-floating-panel')
const nestedComponent = testid('nested-floating-panel')
...
export async function RendersCorrectly() {
  ...
  await expect.element(componentEl).toHaveAttribute('data-scope', 'floating-panel')
}
