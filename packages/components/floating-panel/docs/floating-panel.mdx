...
head:
  - tag: title
    content: Floating Panel- Destyler
...
Floating panel **open** state can be **uncontrolled** or **controlled**. Prefer `defaultOpen` when the machine should own open state. Passing `open` into `machine(...)` is presence-controlled — sync with `onOpenChange` and adapter `context`. Panel `size` / `position` remain programmatic props (not Phase 2 presence-controlled). See [Controllable State](/guide/controllable-state).
