  ...
  return createMachine<MachineContext, MachineState>(
    ...
    },
    {
      ...
      actions: {
        ...
        setPosition(ctx, evt) {
          const diff = subtractPoints(evt.position, ctx.lastEventPosition!)
        ...
        setSize(ctx, evt) {
          if (!ctx.prevSize || !ctx.prevPosition || !ctx.lastEventPosition)
            return
          ...
          const nextPosition = pick(nextRect, ['x', 'y'])
