...
Pagination supports controllable **page** and **pageSize**. Prefer `defaultPage` / `defaultPageSize` for uncontrolled seeds. Passing `page` / `pageSize` into `machine(...)` is presence-controlled — sync with `onPageChange` / `onPageSizeChange` and adapter `context`. See [Controllable State](/guide/controllable-state).
...
const page = 1 // your reactive state
pagination.machine({
  id: "1",
  count: 100,
  page,
