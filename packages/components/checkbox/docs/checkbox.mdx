...
To make a checkbox checked by default **without** taking ownership of state, use `defaultChecked`. Prefer this over seeding with `checked` (which is presence-controlled after Phase 2).
...
To control the checked state from your app, pass `checked` into `machine(...)` and sync with `onCheckedChange` (plus the adapter `context` option). Use `defaultChecked` for uncontrolled demos instead of always passing `checked`. See [Controllable State](/guide/controllable-state).

```ts
const checked = false // your reactive state
checkbox.machine({
  id: "1",
  checked,
...
When the checkbox value changes, the `onCheckedChange` callback is invoked.
