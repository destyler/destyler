...
const disabledComponent = testid('disabled-checkbox')
const readonlyComponent = testid('readonly-checkbox')
const invalidComponent = testid('invalid-checkbox')
const requiredComponent = testid('required-checkbox')
...
export async function RendersCorrectly() {
  ...
  await expect.element(componentEl).toHaveAttribute('data-scope', 'checkbox')
}
