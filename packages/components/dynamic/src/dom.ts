...
export const dom = createScope({
  ...
  getInputId: (ctx: Ctx) => ctx.ids?.input ?? `dynamic:${ctx.id}:input`,
  getClearTriggerId: (ctx: Ctx) => ctx.ids?.clearBtn ?? `dynamic:${ctx.id}:clear-btn`,
  getHiddenInputId: (ctx: Ctx) => ctx.ids?.hiddenInput ?? `dynamic:${ctx.id}:hidden-input`,
  getLabelId: (ctx: Ctx) => ctx.ids?.label ?? `dynamic:${ctx.id}:label`,
  getControlId: (ctx: Ctx) => ctx.ids?.control ?? `dynamic:${ctx.id}:control`,
  getItemId: (ctx: Ctx, opt: ItemProps) => ctx.ids?.item?.(opt) ?? `dynamic:${ctx.id}:tag:${opt.value}:${opt.index}`,
