...
import DynamicInput from './DynamicInput.spec.vue'

describe('given default DynamicInput', () => {
  // @ts-expect-error we return empty object
  window.getComputedStyle = () => ({})
  let wrapper: VueWrapper<InstanceType<typeof DynamicInput>>
  let input: DOMWrapper<HTMLInputElement>
  let tags: DOMWrapper<HTMLElement>[]

  beforeEach(() => {
    wrapper = mount(DynamicInput, { attachTo: document.body })
    tags = wrapper.findAll('[data-destyler-collection-item]')
  })
