...
const disabledComponent = testid('disabled-dynamic')
const readonlyComponent = testid('readonly-dynamic')
const invalidComponent = testid('invalid-dynamic')
const requiredComponent = testid('required-dynamic')
...
export async function RendersCorrectly() {
  ...
  await expect.element(componentEl).toHaveAttribute('data-scope', 'dynamic')
}
