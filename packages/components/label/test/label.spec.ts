...
const disabledComponent = testid('disabled-label')
const readonlyComponent = testid('readonly-label')
const invalidComponent = testid('invalid-label')
const requiredComponent = testid('required-label')
...
export async function RendersCorrectly() {
  ...
  await expect.element(componentEl).toHaveAttribute('data-scope', 'label')
}
