...
const disabledComponent = testid('disabled-qr-code')
const readonlyComponent = testid('readonly-qr-code')
const invalidComponent = testid('invalid-qr-code')
const requiredComponent = testid('required-qr-code')
...
export async function RendersCorrectly() {
  ...
  await expect.element(componentEl).toHaveAttribute('data-scope', 'qr-code')
}
