...
const disabledComponent = testid('disabled-signature')
const readonlyComponent = testid('readonly-signature')
const invalidComponent = testid('invalid-signature')
const requiredComponent = testid('required-signature')
...
export async function RendersCorrectly() {
  ...
  await expect.element(componentEl).toHaveAttribute('data-scope', 'signature')
}
