...
const disabledComponent = testid('disabled-radio')
const readonlyComponent = testid('readonly-radio')
const invalidComponent = testid('invalid-radio')
const requiredComponent = testid('required-radio')
...
export async function RendersCorrectly() {
  ...
  await expect.element(componentEl).toHaveAttribute('data-scope', 'radio')
}
