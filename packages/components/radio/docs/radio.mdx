...
Radio group value can be **uncontrolled** or **controlled**. Prefer `defaultValue` for an uncontrolled seed. Passing `value` into `machine(...)` is presence-controlled — sync with `onValueChange` and adapter `context`. See [Controllable State](/guide/controllable-state).

```ts
const value = "apple" // your reactive state
radio.machine({
  id: "1",
  value,
...
<StyleTable component="radio" nodeType="Root" />

#### Label

<StyleTable component="radio" nodeType="Label" />

#### Item Control

<StyleTable component="radio" nodeType="ItemControl" />

#### Indicator

<StyleTable component="radio" nodeType="Indicator" />
