...
const disabledComponent = testid('disabled-file-upload')
const readonlyComponent = testid('readonly-file-upload')
const invalidComponent = testid('invalid-file-upload')
const requiredComponent = testid('required-file-upload')
...
export async function RendersCorrectly() {
  ...
  await expect.element(componentEl).toHaveAttribute('data-scope', 'file-upload')
}
