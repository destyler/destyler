...
import { compact, getValuePercent, isControlled, isEqual, isPropUserProvided, resolveControllableProp, withControllableProvided } from '@destyler/utils'
...
function isEqualSize(a: ElementSize | null, b: ElementSize | null) {
  ...
  return createMachine<MachineContext, MachineState>(
    ...
    },
    {
      ...
      activities: {
        ...
        trackThumbsSize(ctx) {
          if (ctx.thumbAlignment !== 'contain' || ctx.thumbSize)
            return
          ...
          return trackElementsSize({
            ...
            callback(size) {
              if (!size || isEqualSize(ctx.thumbSize, size))
                return
              ctx.thumbSize = size
            },
          })
