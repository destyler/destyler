...
const disabledComponent = testid('disabled-slider')
const readonlyComponent = testid('readonly-slider')
const invalidComponent = testid('invalid-slider')
const requiredComponent = testid('required-slider')
...
export async function RendersCorrectly() {
  ...
  await expect.element(componentEl).toHaveAttribute('data-scope', 'slider')
}
