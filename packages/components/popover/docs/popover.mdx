...
Popover open state follows the same pattern as dialog. Use `defaultOpen` for an uncontrolled seed; pass `open` + `onOpenChange` (and sync via `context`) when you need controlled open. See [Controllable State](/guide/controllable-state).
