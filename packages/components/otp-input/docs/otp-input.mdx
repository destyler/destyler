...
OTP input value can be **uncontrolled** or **controlled**. Prefer `defaultValue` for an uncontrolled seed. Passing `value` into `machine(...)` is presence-controlled — sync with `onValueChange` and adapter `context`. See [Controllable State](/guide/controllable-state).
...
const value = ["1", "2", ""] // your reactive state
otpInput.machine({
  id: "1",
  value,
