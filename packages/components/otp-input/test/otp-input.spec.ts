...
const disabledComponent = testid('disabled-otp-input')
const readonlyComponent = testid('readonly-otp-input')
const invalidComponent = testid('invalid-otp-input')
const requiredComponent = testid('required-otp-input')
...
export async function RendersCorrectly() {
  ...
  await expect.element(componentEl).toHaveAttribute('data-scope', 'otp-input')
}
