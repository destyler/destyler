...
export class Carousel extends Component<carousel.Context, carousel.Api> {
  ...
  render = () => {
    ...
    if (itemGroupEl)
      this.addCleanup(spreadProps(itemGroupEl, this.api.getItemGroupProps()))

    // 为每个 item 元素设置属性
    const itemEls = itemGroupEl?.querySelectorAll<HTMLElement>('.carousel-item')
    itemEls?.forEach((el, index) => {
      this.addCleanup(spreadProps(el, this.api.getItemProps({ index })))
    })
    ...
    if (scrollToIndexBtn) {
      ...
      scrollToIndexBtn.addEventListener('click', clickHandler)
      this.addCleanup(() => {
        scrollToIndexBtn.removeEventListener('click', clickHandler)
      })
    }
    ...
    if (autoplayBtn) {
      this.addCleanup(spreadProps(autoplayBtn, this.api.getAutoplayTriggerProps()))
    ...
    if (prevBtn)
      this.addCleanup(spreadProps(prevBtn, this.api.getPrevTriggerProps()))
    const nextBtn = rootEl.querySelector<HTMLElement>('.carousel-next')
    if (nextBtn)
      this.addCleanup(spreadProps(nextBtn, this.api.getNextTriggerProps()))

    // 指示器组
    const indicatorGroup = rootEl.querySelector<HTMLElement>('.carousel-indicators')
    if (indicatorGroup) {
      this.addCleanup(spreadProps(indicatorGroup, this.api.getIndicatorGroupProps()))
      const indicatorBtns = indicatorGroup.querySelectorAll<HTMLElement>('.carousel-indicator')
      indicatorBtns.forEach((btn, index) => {
        this.addCleanup(spreadProps(btn, this.api.getIndicatorProps({ index })))
      })
    }
  }
}
