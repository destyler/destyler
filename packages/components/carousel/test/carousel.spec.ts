...
const disabledComponent = testid('disabled-carousel')
const readonlyComponent = testid('readonly-carousel')
const invalidComponent = testid('invalid-carousel')
const requiredComponent = testid('required-carousel')
...
export async function RendersCorrectly() {
  ...
  await expect.element(componentEl).toHaveAttribute('data-scope', 'carousel')
}
