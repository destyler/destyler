...
Carousel **page** can be **uncontrolled** or **controlled**. Prefer `defaultPage` for an uncontrolled seed. Passing `page` into `machine(...)` is presence-controlled — sync with `onPageChange` and adapter `context`. See [Controllable State](/guide/controllable-state).
...
const page = 0 // your reactive state
carousel.machine({
  id: "1",
  slideCount: 5,
  page,
