...
export function isValidDate(value: DateValue) {
  return !Number.isNaN(value.day) && !Number.isNaN(value.month) && !Number.isNaN(value.year)
}

export function ensureValidCharacters(value: string, separator: string) {
  return value
    .split('')
...
export const defaultTranslations: IntlTranslations = {
  dayCell(state) {
    if (state.unavailable)
      return `Not available. ${state.formattedDate}`
    if (state.selected)
      return `Selected date. ${state.formattedDate}`
