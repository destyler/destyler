...
const disabledComponent = testid('disabled-calendar')
const readonlyComponent = testid('readonly-calendar')
const invalidComponent = testid('invalid-calendar')
const requiredComponent = testid('required-calendar')
...
export async function RendersCorrectly() {
  ...
  await expect.element(componentEl).toHaveAttribute('data-scope', 'calendar')
}
