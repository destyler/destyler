...
To set the initial value **without** taking ownership of state, pass `defaultValue`. Prefer this over seeding with `value` (presence-controlled after Phase 2).

```ts
const [state, send] = useMachine(
  calendar.machine({
    defaultValue: [calendar.parse("2021-01-01")],
  }),
)
```
...
Calendar supports controllable **value** and **open**. Prefer `defaultValue` / `defaultOpen` for uncontrolled seeds. Passing `value` / `open` into `machine(...)` is presence-controlled — sync with `onValueChange` / `onOpenChange` and adapter `context`. See [Controllable State](/guide/controllable-state).
...
const value = [calendar.parse("2021-01-01")] // your reactive state
calendar.machine({
  id: "1",
  value,
...
we recommended setting the `defaultOpen` property to `true` and `closeOnSelect` to `false`.
