...
Edit supports controllable **value** (and edit **mode** via `edit` presence / soft-deprecated `edit.controlled`). Prefer `defaultValue` for an uncontrolled text seed. There is no `defaultEdit` — use `'edit.controlled': false` (or omit `edit`) to start uncontrolled in edit mode. See [Controllable State](/guide/controllable-state).
...
const value = "Hello World" // your reactive state
edit.machine({
  id: "1",
  value,
