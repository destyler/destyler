...
Edit supports controllable **value** (and edit **mode** via `edit` presence). Prefer `defaultValue` for an uncontrolled text seed. There is no `defaultEdit` — omit `edit` and enter edit mode via gestures for uncontrolled usage. See [Controllable State](/guide/controllable-state).
