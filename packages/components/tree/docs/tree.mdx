...
const [state, send] = useMachine(
  tree.machine({
    defaultExpandedValue: ["node_modules/unocss"],
  }),
)
```
...
const [state, send] = useMachine(
  tree.machine({
    defaultSelectedValue: ["node_modules/unocss"],
  }),
)
```
...
Tree supports controllable **expandedValue** and **selectedValue**. Prefer `defaultExpandedValue` / `defaultSelectedValue` for uncontrolled seeds. Passing those keys into `machine(...)` is presence-controlled — sync with `onExpandedChange` / `onSelectionChange` and adapter `context`. See [Controllable State](/guide/controllable-state).
...
const expandedValue = ["node_modules"] // your reactive state
tree.machine({
  id: "1",
  expandedValue,
