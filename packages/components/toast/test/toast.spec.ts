...
const disabledComponent = testid('disabled-toast')
const readonlyComponent = testid('readonly-toast')
const invalidComponent = testid('invalid-toast')
const requiredComponent = testid('required-toast')
...
export async function RendersCorrectly() {
  ...
  await expect.element(componentEl).toHaveAttribute('data-scope', 'toast')
}
