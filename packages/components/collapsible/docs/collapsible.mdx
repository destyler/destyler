...
Collapsible open state can be **uncontrolled** or **controlled**. Prefer `defaultOpen` when the machine should own open state. Passing `open` into `machine(...)` is presence-controlled — sync with `onOpenChange` and adapter `context`. See [Controllable State](/guide/controllable-state).
