...
const disabledComponent = testid('disabled-splitter')
const readonlyComponent = testid('readonly-splitter')
const invalidComponent = testid('invalid-splitter')
const requiredComponent = testid('required-splitter')
...
export async function RendersCorrectly() {
  ...
  await expect.element(componentEl).toHaveAttribute('data-scope', 'splitter')
}
