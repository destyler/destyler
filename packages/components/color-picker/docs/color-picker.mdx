...
Color picker supports controllable **value** and **open**. Prefer `defaultValue` / `defaultOpen` for uncontrolled seeds. Passing `value` / `open` into `machine(...)` is presence-controlled — sync with `onValueChange` / `onOpenChange` and adapter `context`. See [Controllable State](/guide/controllable-state).
