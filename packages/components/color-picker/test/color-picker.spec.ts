...
const disabledComponent = testid('disabled-color-picker')
const readonlyComponent = testid('readonly-color-picker')
const invalidComponent = testid('invalid-color-picker')
const requiredComponent = testid('required-color-picker')
...
export async function RendersCorrectly() {
  ...
  await expect.element(componentEl).toHaveAttribute('data-scope', 'color-picker')
}
