...
:::tip
The `defaultValue` / `value` property must be an array of strings.
...
Select supports controllable **value** and **open**. Use `defaultValue` / `defaultOpen` for uncontrolled seeds. Pass `value` / `open` and sync with `onValueChange` / `onOpenChange` when your app owns state. See [Controllable State](/guide/controllable-state).
