...
const disabledComponent = testid('disabled-select')
const readonlyComponent = testid('readonly-select')
const invalidComponent = testid('invalid-select')
const requiredComponent = testid('required-select')
...
export async function RendersCorrectly() {
  ...
  await expect.element(componentEl).toHaveAttribute('data-scope', 'select')
}
