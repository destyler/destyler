...
Content menu open state uses the same menu machine contract. Prefer `defaultOpen` for an uncontrolled seed; pass `open` + `onOpenChange` (and sync via `context`) when controlled. See [Controllable State](/guide/controllable-state).
