...
Menu open state can be **uncontrolled** or **controlled**. Prefer `defaultOpen` for an uncontrolled seed. Passing `open` into `machine(...)` is presence-controlled — sync with `onOpenChange` and adapter `context`. See [Controllable State](/guide/controllable-state).
