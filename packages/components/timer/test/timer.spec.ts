...
const disabledComponent = testid('disabled-timer')
const readonlyComponent = testid('readonly-timer')
const invalidComponent = testid('invalid-timer')
const requiredComponent = testid('required-timer')
...
export async function RendersCorrectly() {
  ...
  await expect.element(componentEl).toHaveAttribute('data-scope', 'timer')
}
