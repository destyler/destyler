...
const disabledComponent = testid('disabled-dialog')
const nestedComponent = testid('nested-dialog')
...
export async function RendersCorrectly() {
  ...
  await expect.element(componentEl).toHaveAttribute('data-scope', 'dialog')
}
