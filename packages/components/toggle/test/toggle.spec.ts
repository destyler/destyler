...
const disabledComponent = testid('disabled-toggle')
const readonlyComponent = testid('readonly-toggle')
const invalidComponent = testid('invalid-toggle')
const requiredComponent = testid('required-toggle')
...
export async function RendersCorrectly() {
  ...
  await expect.element(componentEl).toHaveAttribute('data-scope', 'toggle')
}
