...
function normalizeEventPoint(event: PointerEvent) {
  ...
  while (frame) {
    ...
    const left = iframeRect.left + (frame.clientLeft + Number.parseFloat(css.paddingLeft))
    const top = iframeRect.top + (frame.clientTop + Number.parseFloat(css.paddingTop))
