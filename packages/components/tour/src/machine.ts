      ...
      actions: {
        ...
        validateSteps(ctx) {
          ...
          ctx.steps.forEach((step) => {
            if (ids.has(step.id)) {
              throw new Error(`[destyler/tour] Duplicate step id: ${step.id}`)
            }

            if (step.target == null && step.type == null) {
              throw new Error(`[destyler/tour] Step ${step.id} has no target or type. At least one of those is required.`)
            }
