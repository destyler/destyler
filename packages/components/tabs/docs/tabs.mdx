...
<FrameworkLayout>
  ...
  <script lang="ts">
    ...
    const id = $props.id()
    const [state, send] = useMachine(tabs.machine({ id, defaultValue: 'item-1' }))
...
Tabs value can be **uncontrolled** or **controlled**. Prefer `defaultValue` for an uncontrolled seed. Passing `value` into `machine(...)` is presence-controlled — sync with `onValueChange` and adapter `context`. See [Controllable State](/guide/controllable-state).
...
const value = "tab-1" // your reactive state
tabs.machine({
  id: "1",
  value,
