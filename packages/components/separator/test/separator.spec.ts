...
const disabledComponent = testid('disabled-separator')
const readonlyComponent = testid('readonly-separator')
const invalidComponent = testid('invalid-separator')
const requiredComponent = testid('required-separator')
...
export async function RendersCorrectly() {
  ...
  await expect.element(componentEl).toHaveAttribute('data-scope', 'separator')
}
