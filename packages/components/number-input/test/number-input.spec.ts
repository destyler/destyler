...
const disabledComponent = testid('disabled-number-input')
const readonlyComponent = testid('readonly-number-input')
const invalidComponent = testid('invalid-number-input')
const requiredComponent = testid('required-number-input')
...
export async function RendersCorrectly() {
  ...
  await expect.element(componentEl).toHaveAttribute('data-scope', 'number-input')
}
