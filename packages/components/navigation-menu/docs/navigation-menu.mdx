...
Prefer `defaultValue` to open a menu initially without taking ownership. Pass `value` + `onValueChange` when your app owns state (presence-controlled):

```ts
const [state, send] = useMachine(
  navigationMenu.machine({
    defaultValue: 'products', // Opens the products menu initially (uncontrolled seed)
...
Navigation menu openness is encoded as **value** (which trigger is active). Prefer `defaultValue` for an uncontrolled seed. Passing `value` into `machine(...)` is presence-controlled — sync with `onValueChange` and adapter `context`. See [Controllable State](/guide/controllable-state).
...
const value = "products" // your reactive state
navigationMenu.machine({
  id: "1",
  value,
  onValueChange(details) {
