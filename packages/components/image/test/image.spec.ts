...
const disabledComponent = testid('disabled-image')
const readonlyComponent = testid('readonly-image')
const invalidComponent = testid('invalid-image')
const requiredComponent = testid('required-image')
...
export async function RendersCorrectly() {
  ...
  await expect.element(componentEl).toHaveAttribute('data-scope', 'image')
}
