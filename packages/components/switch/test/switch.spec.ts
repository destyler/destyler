...
const disabledComponent = testid('disabled-switch')
const readonlyComponent = testid('readonly-switch')
const invalidComponent = testid('invalid-switch')
const requiredComponent = testid('required-switch')
...
export async function RendersCorrectly() {
  ...
  await expect.element(componentEl).toHaveAttribute('data-scope', 'switch')
}
