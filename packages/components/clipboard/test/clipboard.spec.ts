...
const disabledComponent = testid('disabled-clipboard')
const readonlyComponent = testid('readonly-clipboard')
const invalidComponent = testid('invalid-clipboard')
const requiredComponent = testid('required-clipboard')
...
export async function RendersCorrectly() {
  ...
  await expect.element(componentEl).toHaveAttribute('data-scope', 'clipboard')
}
