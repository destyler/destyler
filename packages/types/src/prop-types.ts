import type { CSSProperties, HTMLAttributes, IntrinsicElements } from './jsx'
...
export type NormalizeProps<T extends PropTypes> = {
  [K in keyof T]: (props: K extends keyof IntrinsicElements ? DataAttr & IntrinsicElements[K] : never) => T[K]
} & {
  element: (props: DataAttr & HTMLAttributes<HTMLElement> & Record<string, any>) => T['element']
  style: CSSProperties
}
